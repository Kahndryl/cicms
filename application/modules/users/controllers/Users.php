...
class Users extends CI_Controller{
	...
	public function __construct(){
		...
		//echo is_user_login();
		if(!in_array($method,array('register','forgot'))){
...
*/
	...
	public function edit_detail(){
		...
		if ($this->form_validation->run() === FALSE){	
			...
			//$this->session->set_flashdata('image', 'Profile updated succesfully');
	...
	public function edit_bank(){
		...
		if ($this->form_validation->run() === FALSE){	
			...
			$this->session->set_flashdata('bank_info', 'Bank Detail updated succesfully');
			redirect('/users/bank_detail');
		}
		$editdetail = $this->users_model->getBankAccountDetail();
		$data['editdetail'] = $editdetail;
		$data['title'] = 'Bank Detail';
	...
	/* display profile detail */
        ...
        $amount = $this->users_model->getTransfer();
	...
	public function referallink(){
		...
		$query = $this->users_model->get_refferer($user_id);
	...
	public function update_bitcoin(){
		...
		/*  Xapo */
	public function xapo(){
		$xapo = $this->users_model->getXapo();
	   // print_r($bitCoin); die;
		$data['xapo'] = $xapo;
		$data['title'] = 'Xapo';
		$data['view'] = 'xapo';
		...
		/* update or insert xapo address */
	public function update_xapo(){
		$updatexapo = $this->users_model->updatexapo();
		redirect('users');
	}
       ...
       return true;
        ...
        public function forgot(){
			...
			if ($this->form_validation->run() === FALSE){	
				...
				$this->users_model->forgot_update_password($email,$password);
				//echo $password; die;
				$header = 'Your New Forgot Password.';
				$user= $this->users_model->get_user_email_forgot($email);
				...
				if(!empty($user) && ($user->email == $email)){
					...
					@send_mail($this->input->post('email'),$header,$message);
					$this->session->set_flashdata('message', 'A mail has been sent to your email');
					}
					else{
						$this->session->set_flashdata('message', 'Your email not registered with us');
						redirect('/user/forgot/');
					}
			}
	    $data['view'] = 'users/forgot_password';
