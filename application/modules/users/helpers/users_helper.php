...
function get_user_offer_help_received($user_id = -1){
	$ci = & get_instance();
	$sql = "SELECT SUM(initial_amount) as initial_amount FROM offers WHERE user_id = ".$user_id." AND status = 1";
	$query = $ci->db->query($sql);
	$result = $query->row();
	if(empty($result)){
		return 0;
...
# return guider user_id if guider is manager
function get_user_manager($user_id = -1){
	$ci = & get_instance();
	//$sql = "SELECT * FROM users WHERE is_manager = 1 AND contact IN (SELECT guider_contact FROM users WHERE user_id = $user_id)";
	$sql = "SELECT * FROM users WHERE contact IN (SELECT guider_contact FROM users WHERE user_id = $user_id)";
	//echo $sql.'<br/>';
	$query = $ci->db->query($sql);
	$result = $query->row();
